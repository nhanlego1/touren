...
define('HOST_TERM_ID', 36);
define('TOUR_TERM_ID', 21);
/**
...
function tao_taxonomysHost2()
{
     ...
     */
    $labels = array(
        'name' => 'Theme & Category (Host)',
        'singular' => 'Theme & Category (Host)',
        'menu_name' => 'Theme & Category (Host)'
    );
...
function tao_taxonomysHost3()
{
    ...
    $labels = array(
        'name' => 'Language (Host)',
        'singular' => 'Language (Host)',
        'menu_name' => 'Language (Host)'
    );
                    ...
                    <div class="col-xs-6 text-right">
                        <a href="<?php echo get_permalink($post->ID); ?>">
                            <button class="btn my-btn"><span class="glyphicon glyphicon-search"
                                                             aria-hidden="true"></span> Tour Details
                            </button>
                        </a>
                    </div>
                </div>
...
function create_custom_post_type()
{
     ...
     */
    $label = array(
        'name' => 'Host Guide', //Tên post type dạng số nhiều
        'singular_name' => 'Host Guide' //Tên post type dạng số ít
    );
    ...
    $args = array(
        ...
        'description' => 'Create description host guide', //Mô tả của post type
        'supports' => array(
            'title',
            'editor',
            'excerpt',
    ...
    register_post_type('hostguide', $args); //Tạo post type với slug tên là sanpham và các tham số trong biến $args ở trên
...
/**
 * function get post meta by key
 */
function _get_post_meta($post_id, $key){
    global $wpdb;
    $results = $wpdb->get_results('SELECT * FROM postmeta WHERE post_id = $post_id AND meta_key=$key', OBJECT );
    var_dump($results);
    return $results;
}